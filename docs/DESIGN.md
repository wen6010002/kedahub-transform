...
本文档写于 2026-09-13，视觉与交互语言为「山径 · 印记」（窄栏杂志排印风），**该方向已被用户否决**。定稿方向见 `REPORT.md`：

- **首页维持课搭原有的白底橙宽幅信息流（资料馆）**，新增的只有一套卡通「路线积木」
- 技术选型由 Java（Spring AI Alibaba）改为**全 TypeScript 单代码库**
- 页面编号改为 01–10：旧 02-direction→03、旧 03-survey→04、旧 04-generating→05、旧 05-roadmap→06、旧 06-node→07、旧 07-library→08、旧 08-cohort→09、旧 09-pricing→10
...
**说明**：本章结论全部来自 research/community-cohort.md（80+ 带来源引用），此处只留决策与数字，出处见调研原文。
...
**说明**：每节开头给出对应原型文件；规格写到"开发可直接照做"的程度；文案原则：克制的中文、不用感叹号、不制造焦虑、把承诺写具体。
...
**说明**：本章是 2.0 的技术灵魂，结论来自 research/agent-tech.md（60+ 带来源引用）。总原则先行，四类能力逐个展开。
...
**说明**：复用 vs 新建的完整判决见 research/baseline-assets.md。本章只列 2.0 新建与关键改造（Prisma 风格）。
...
**说明**：总策略：**在 campus-market 代码库上渐进演进，不重写**。新建 kedahub-next 仓库先承载设计资产（本文档+原型+调研），代码转型开工时将其作为 campus-market 的 fork 起点或直接在原仓库开 v2 长分支 ⚠️（倾向：原仓库拉 transform/v2 分支，稳定后合入 main——保 CI/测试/部署链路连续性）。
